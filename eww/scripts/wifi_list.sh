...
echo "$networks" | while IFS=: read -r ssid signal security active; do
  if [ -n "$ssid" ]; then
    ...
    if echo "$known_networks" | grep -qx "$ssid"; then
      ...
      cmd="eww update auth_ssid=\"$ssid\" auth_pass=\"\" auth_error=\"\" && eww open --toggle wifi_auth --screen \"$(hyprctl monitors -j | jq -r '.[] | select(.focused == true) | if .model != "" then .model else .description end' | awk '{print $1}')\" --arg screen=\"$(hyprctl monitors -j | jq -r '.[] | select(.focused == true) | if .model != "" then .model else .description end' | awk '{print $1}')\""
    fi
